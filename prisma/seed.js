...
async function main() {
  ...
  const equipSkills = await prisma.$transaction(equipSkillDefs.map((d) => prisma.skill.create({ data: d })));
  ...
  await prisma.$transaction(
    equipmentDefs.map(({ grantedSkillName, ...e }) =>
  ...
  console.log(`Seeded ${kido.all.length} kidō, ${stageDefs.length} story stages and ${equipmentDefs.length} equipment.`);
